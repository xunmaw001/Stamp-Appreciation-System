<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.fenxiangfengmian" label="分享封面" prop="fenxiangfengmian">
          <file-upload
          tip="点击上传分享封面"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.fenxiangfengmian" label="分享封面" prop="fenxiangfengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fenxiangfengmian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="分享简介" prop="fenxiangjianjie">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="分享简介"
                  v-model="ruleForm.fenxiangjianjie" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.fenxiangjianjie" label="分享简介" prop="fenxiangjianjie">
                    <span>{{ruleForm.fenxiangjianjie}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fenxiangfengmian = true;
          ...
          if(o=='fenxiangjianjie'){
            this.ruleForm.fenxiangjianjie = obj[o];
	    this.ro.fenxiangjianjie = true;
            continue;
          }
	...
	this.ruleForm.fenxiangneirong = this.ruleForm.fenxiangneirong.replace(reg,'../../../ssm34f5g/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gerenfenxiang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gerenfenxiangCrossAddOrUpdateFlag = false;
						 this.parent.search();
