<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="邮票分类" prop="youpiaofenlei">
          <el-input v-model="ruleForm.youpiaofenlei" 
              placeholder="邮票分类" clearable  :readonly="ro.youpiaofenlei"></el-input>
        </el-form-item>
        <div v-else>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youpiaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youpiaofenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
